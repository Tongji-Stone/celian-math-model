...
1. Kruskal--Wallis 检验策略间 `SOH_200` 差异（论文主检验，H=12.3346），配合 Holm 校正的 Mann--Whitney 两两比较。`Fade_200` 另有补充检验（H=14.79），两个数不要混用。Holm 不显著不等于禁止推荐。
...
- 全题口径：`A/口径.md`（充电时间、两个 Kruskal、推荐语言）
- 论文：机制与口径对齐时按 `A/口径.md` 修订 `paper/`；组间检验的原始计算仍以本目录 markdown 为准
